...
Converge repos to the desired state defined in a manifest. This is the primary command for managing per-repo installations — it handles adding repos to the manifest, provisioning new repos, repairing component drift (workflow, thin callers, variables, secrets, pipeline schedules — including reactivating GitLab schedules that exist but are disabled), repairing scaffold content drift (including structural rewrites of GitLab dispatch templates at an unchanged ref), and upgrading scaffold refs.
