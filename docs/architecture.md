...
- GitHub Packages for sandboxed installs: the job's Actions workflow token is preserved as `FULLSEND_WORKFLOW_TOKEN`, readable only by provider credential expansion, and a repo-level provider binds it to `npm.pkg.github.com` (read-only, enforced) with the tarball CDN allowlisted without a credential; forge identity stays the minted App token, which cannot read packages owned by another org ([ADR 0114](ADRs/0114-github-packages-via-host-bound-workflow-token-provider.md)).
