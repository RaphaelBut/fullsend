...
- `FULLSEND_GITLAB_POLLER_TOKEN`, `FULLSEND_GITLAB_ANALYST_TOKEN`, `FULLSEND_GITLAB_CODER_TOKEN`, `FULLSEND_GITLAB_ROLE_<NAME>_TOKEN`, `FULLSEND_GITLAB_ROLE_MIGRATION`, `FULLSEND_GITLAB_ROLE_REGISTRY` — reserved names for the registered-role credential contract ([gitlab-role-credentials.md](../../contributing/gitlab-role-credentials.md)). Not provisioned by `repos install`; absence is not a health failure. The registry variable is administrator-controlled policy (not repository content) and must not contain secret values.
